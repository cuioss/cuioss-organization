...
* **`group`**: Groups runs by workflow name and branch reference
* **`cancel-in-progress: true`**: When a new run starts, any in-progress run in the same group is cancelled

This means the first run (typically `push`) gets cancelled when the second (`pull_request`) starts, ensuring only one run completes per commit.
