...
* **`pull_request` to `main`**: Verifies all pull requests targeting the main branch.
...
The `concurrency` block prevents duplicate runs:

* **`group`**: Groups runs by workflow name and branch name
** `github.head_ref` returns the source branch name for PRs (e.g., `feature/my-branch`)
** `github.ref_name` returns the branch name for push events (e.g., `feature/my-branch`)
** The `||` fallback ensures both events use the same branch name as the group key
* **`cancel-in-progress: true`**: When a new run starts, any in-progress run in the same group is cancelled

This ensures both push and PR events for the same branch are in the same concurrency group, so only one run completes per commit.
