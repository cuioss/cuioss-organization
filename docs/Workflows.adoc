...
We use two mechanisms to prevent duplicate runs:

==== 1. Conditional Job Execution (Primary)
...
==== 2. Concurrency (Secondary)

The `concurrency` block cancels outdated runs when you push multiple commits quickly:

* **`group`**: Groups runs by workflow name and branch name
* **`cancel-in-progress: true`**: Newer runs cancel older in-progress runs

This ensures only the latest commit is verified when rapidly pushing to a branch.
