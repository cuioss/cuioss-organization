...
[source,yaml]
----
# Repos to update after release (with optional version-property hints)
consumers:
  - cui-java-tools
  - cui-core-ui-model
  - cuioss-parent-pom:version.cui.java.parent  # hint: update property directly

# What artifact to propagate
...
* `parent` — Updates the `<parent><version>` in the consumer's root `pom.xml`
* `dependency` — Updates `<dependency><version>` or the referenced `<properties>` value across all POM files

**Version property hints:** For `scope: dependency`, each consumer entry **must** include a `:property-name` suffix (e.g., `cuioss-parent-pom:version.cui.java.parent`). The script directly updates the named Maven property across all POM files. For `scope: parent`, no hint is needed.
...
jobs:
  auto-merge:
    uses: cuioss/cuioss-organization/.github/workflows/reusable-dependabot-auto-merge.yml@89ce95e04b1e15076b0c8d78d8112fa432d9d162 # v0.3.11
    permissions:
      contents: write
      pull-requests: write
----
