...
GitHub App that posts the PR-Agent reviews (`reusable-cuioss-review-bot.yml`) across cuioss
repositories.
...
central configuration from the settings repository `cuioss/cuioss-review-bot`, and if the token
cannot read that repository it skips the global configuration *silently, with no error*.

PR-Agent asks for that repository under its hard-coded name `pr-agent-settings` and reaches
`cuioss/cuioss-review-bot` through GitHub's rename redirect. No repository named
`pr-agent-settings` may therefore ever be created in the organization: it would break the
...
`repositories` MUST list the settings repository `cuioss-review-bot` alongside the current repository:
...
* xref:Workflows.adoc[Workflows.adoc] — the `cuioss-review-bot Review` reusable workflow.
* xref:automatic-review/pr-agent.md[automatic-review/pr-agent.md] — review anatomy and signal vs. noise.
* https://github.com/cuioss/cuioss-review-bot[`cuioss/cuioss-review-bot`] — the central configuration and the setup's recorded learnings.
