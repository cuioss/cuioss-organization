...
The reusable workflows use a three-tier precedence, highest first:
...
*off* that a caller asked to enable.

This allows:
- Zero-configuration usage when project.yml is fully populated
- Per-caller overrides via explicit inputs for keys project.yml does not set
- Graceful fallback to sensible defaults

NOTE: Boolean keys resolved this way (`maven-build.npm-cache`,
`sonar.skip-on-dependabot`, `pyprojectx.upload-artifacts-on-failure`) report an
empty value when unset, so that an explicit `false` is distinguishable from
"not configured". The *effective* default when both sources are silent is
unchanged — it is the workflow input default listed below.
