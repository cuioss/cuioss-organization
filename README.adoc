...
permissions:
  contents: read
  pull-requests: read  # the reusable workflow's gate reads open PRs
  actions: read        # the covering-run-check job verifies a covering run exists
...
Org-wide automated PR reviewers (CodeRabbit, Sourcery): how to read their output (signal vs.
noise), how they are configured, and how they feed plan-marshall's automated triage.

See: link:docs/automatic-review/README.md[Automatic Review Documentation]
